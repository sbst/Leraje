...
Table::Table() : lastId(0), table(new UsersRows())
{}
...
bool Table::FromJson(string json, UsersData& row)
{
  bool result = false;
  try
  {
...
bool Table::ToJson(uint32_t id, UsersData* row, string& json)
{
  bool result = false;
  try
  {
...
void Table::OutcomeData(uint32_t id, UsersData* data, boost::property_tree::ptree& tree)
{
  tree.put("id", id);
...
void Table::IncomeData(boost::property_tree::ptree* tree, UsersData& data)
{
